...
使用图像: image/OpenCV三周入门手册/feature_map.png
对同一张图分别使用 Harris、Shi-Tomasi、FAST 三种角点检测器进行检测，
并把 原图 + 三种检测结果 组成 2x2 的对比图保存为 image/OpenCV三周入门手册/feature_corners_compare.png

说明:
    本脚本**不使用命令行参数**，直接使用相对于脚本位置的固定输入/输出路径。
"""
...
def main() -> None:
    # 使用脚本文件位置作为基准，直接使用固定输入/输出路径（不使用命令行参数）
    script_dir = os.path.dirname(os.path.abspath(__file__))
    in_path = os.path.normpath(os.path.join(script_dir, '..', '..', 'image', 'OpenCV三周入门手册', 'feature_map.png'))
    out_path = os.path.normpath(os.path.join(script_dir, '..', '..', 'image', 'OpenCV三周入门手册', 'feature_corners_compare.png'))
