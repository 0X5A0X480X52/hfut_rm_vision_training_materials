...
- [C++ 官方文档](https://en.cppreference.com/w/)：权威的 C++ 参考手册，**强烈推荐**！
- [菜鸟教程 C++ 教程](https://www.runoob.com/cplusplus/cpp-tutorial.html)：提供大量实例与清晰解释，从基础语法到高级特性，循序渐进引导学习。
- [C++ Primer Plus（第6版）中文版](https://book.douban.com/subject/10789789/)：经典 C++ 入门书籍，内容详实全面。
- [LearnCpp.com](https://www.learncpp.com/)：优秀的英文 C++ 教程网站，讲解清晰详细。

**书籍**：

- **《C++ Primer（第5版）》**：全面介绍 C++ 语言，从基础到高级，适合系统学习。
- **《C++ Primer Plus（第6版）》**：循序渐进，适合初学者，配有大量示例和练习。
...
- 菜鸟教程：
  ...
  - [C++ 多继承](https://www.runoob.com/cplusplus/cpp-multilevel-inheritance.html)
...
|     天数    | 主题            | 核心知识点                                                                     | 学习目标                   | 小任务                             |
| :-------: | :------------ | :------------------------------------------------------------------------ | :--------------------- | :------------------------------ |
|   **D1**  | C++ 程序结构与输入输出 | - `iostream` (`cin`, `cout`)<br>- 命名空间 `namespace std`<br>- C 与 C++ 的语法差异 | 能独立编写 C++ 的基本程序，理解编译方式 | 将 C 语言的输入输出程序改写为使用 `cin`/`cout` |
| **D2–D3** | 类与对象初步        | - `class` 定义与访问控制<br>- 构造函数与析构函数<br>- 成员函数与数据成员                           | 能定义类、实例化对象，理解封装思想      | 定义 `Student` 类并打印学生信息           |
| **D4–D5** | 构造函数进阶与对象生命周期 | - 构造函数重载<br>- 拷贝构造函数与赋值操作符<br>- 析构函数自动调用                                  | 理解对象创建与销毁过程            | 实现一个带拷贝构造的 `Book` 类             |
| **D6–D7** | 组合与对象成员       | - 类中包含类（has-a 关系）<br>- 初始化列表 `: member(value)`<br>- 对象成员的构造顺序             | 学会组织多个类之间的关系           | 创建 `Student` 与 `Score` 类的组合关系   |
...
|      天数     | 主题       | 核心知识点                                                       | 学习目标            | 小任务                                         |
| :---------: | :------- | :---------------------------------------------------------- | :-------------- | :------------------------------------------ |
|  **D8–D9**  | 继承机制与派生类 | - `public` / `protected` / `private` 继承<br>- 构造函数与析构函数的继承关系 | 理解继承的结构与代码复用    | 创建 `Person` 基类与 `Teacher`、`Student` 子类      |
| **D10–D11** | 虚函数与多态   | - 虚函数 `virtual`<br>- 重写 `override`<br>- 动态绑定与静态绑定           | 掌握多态与动态行为       | 用多态实现“打印不同类型用户信息”                           |
| **D12–D13** | 抽象类与接口设计 | - 纯虚函数 `= 0`<br>- 抽象基类<br>- 接口分离原则                          | 能设计抽象接口，构建可扩展结构 | 设计一个 `Shape` 接口，派生 `Circle` 与 `Rectangle` 类 |
|   **D14**   | 友元与运算符重载 | - `friend` 函数与类<br>- 常见运算符重载：`+`, `==`, `<<`                | 学会让类更自然地表达语义    | 为 `Complex` 类重载加法和输出运算符                     |
...
|      天数     | 主题               | 核心知识点                                                                     | 学习目标             | 小任务                                        |
| :---------: | :--------------- | :------------------------------------------------------------------------ | :--------------- | :----------------------------------------- |
| **D15–D16** | 文件与流操作           | - `fstream` 读写文本/二进制文件<br>- 数据持久化与文件格式设计                                  | 能使用文件保存与加载对象信息   | 实现一个简单的学生信息存档系统                            |
| **D17–D18** | 模块化与头文件管理        | - `.h` 与 `.cpp` 的分离编译<br>- 头文件防卫宏 `#ifndef` / `#pragma once`<br>- 多文件工程组织 | 理解项目结构与代码重用      | 将学生系统拆分为多个 `.h` + `.cpp` 文件                |
|   **D19**   | 简单项目管理与 CMake 实践 | - 基本 CMakeLists.txt<br>- 多源文件编译与链接<br>- 工程构建流程                            | 能独立构建一个小型 C++ 项目 | 使用 CMake 构建完整项目                            |
| **D20–D21** | 综合项目实践           | - 类设计与模块划分<br>- 文件存储与数据封装<br>- 用户输入交互                                     | 综合应用所有知识，独立完成项目  | **学生成绩管理系统（命令行版）** 或 **Flappy Bird 控制台游戏** |
...
| 阶段  | 学习主题         | 能力目标       |
| --- | ------------ | ---------- |
| 第一周 | C++ 基础语法与类对象 | 能用类封装数据与行为 |
| 第二周 | 继承与多态机制      | 掌握面向对象核心思想 |
| 第三周 | 工程化与综合项目     | 能构建完整可编译项目 |
...
| **E5** | 文件与流操作进阶 | ⭐⭐ | 完成 D15-16 后 | ⭐⭐⭐ |
| **E6** | 现代 C++11-17 特性 | ⭐⭐⭐⭐ | 全部基础完成后 | ⭐⭐⭐⭐⭐ |
...
- [菜鸟教程 - C++ Lambda 表达式](https://www.runoob.com/cplusplus/cpp-lambda.html)
...
- 字符串流（`stringstream`, `istringstream`, `ostringstream`）
...
- 使用 `istringstream` 解析 CSV 格式数据
- 使用 `ostringstream` 格式化输出报表
...
- [菜鸟教程 - C++11 新特性](https://www.runoob.com/cplusplus/cpp-11.html)
...
std::vector<std::string> createLargeVector() {
    ...
    return vec; // 使用移动语义，无需拷贝
}
```

**任务 3**：`std::optional` 处理可能失败的操作

```cpp
std::optional<Student> findStudent(const std::string& name) {
    // 找到返回 Student，否则返回 std::nullopt
}
```
