...
- 相关博客：[C++ 对象组合](https://www.echo.cool/docs/language/cpp/cpp-object-oriented-advanced/cpp-object-combination/)
...
### Day 20-21.ex：综合项目实践② - 控制台 Flappy Bird（OOP版）

#### 📘 21.1 项目概述

本项目旨在使用 **C++ 面向对象编程**（OOP）实现一个控制台版本的 Flappy Bird 小游戏。Flappy Bird 是一款经典的横版闯关游戏，玩家通过控制小鸟跳跃，躲避不断移动的障碍物，并尽可能获取更高的分数。

本项目不仅考察 **C++ 面向对象设计能力**，还涉及 **控制台图形绘制、物理模拟、用户输入处理、游戏循环设计、碰撞检测和分数系统实现** 等综合编程技能。

通过本项目，希望同学们应掌握以下能力：

1. 将游戏元素抽象为类（小鸟、障碍物、游戏管理器）并进行封装。
2. 实现小鸟的运动逻辑，包括 **重力模拟、跳跃操作**。
3. 使用 **定时器和主循环** 控制游戏帧率。
4. 通过 **字符界面** 实现游戏场景的绘制。
5. 处理用户 **非阻塞输入**（空格键跳跃、退出游戏）。
6. 实现障碍物的 **随机生成与移动**。
7. 完成 **碰撞检测和分数统计**。
8. 实现 **游戏结束判定和界面清屏刷新**。
...
1. **小鸟的跳跃与运动**
   ...
   - 使用 `Bird` 类封装小鸟的状态（纵坐标 y 和速度 velocity）。
   - 小鸟每一帧受 **重力加速度** 作用向下运动。
   - 当用户按下空格键时，小鸟获得一个向上的速度，实现跳跃效果。

2. **障碍物的生成与移动**

   - 使用 `Obstacle` 类封装障碍物属性（x 坐标、缺口位置 gapY、缺口大小 gapSize）。
   - 每帧向左移动，当障碍物超出屏幕左侧时，重新生成在屏幕右侧并随机调整缺口位置。

3. **碰撞检测**

   - 小鸟与障碍物的碰撞判定：小鸟的坐标落在障碍物的非缺口区域即判定碰撞。
   - 小鸟触底或飞出顶部同样判定游戏结束。
...
5. **控制台界面与刷新**
   ...
   - 使用 **字符界面绘制**游戏场景，包括小鸟和障碍物。
   - 每帧刷新屏幕，保持游戏动态效果。
   - 使用跨平台清屏方法（Windows: `system("cls")`，Linux/Mac: `system("clear")`）。

6. **非阻塞输入处理**

   - 实现非阻塞检测键盘输入，实现实时跳跃响应。
   - 支持退出游戏操作（如按 `Q` 键退出）。

7. **游戏主循环设计**

   - 游戏主循环负责更新小鸟和障碍物状态、检测碰撞、绘制游戏界面，并处理用户输入。
   - 循环频率控制游戏速度，保证体验平滑。
...
public:
    ...
    Bird(int startY);
    void jump();    // 跳跃操作
    void update();  // 更新物理状态（重力）
};
```

- `update()`：每一帧根据速度和重力更新 y 坐标
- `jump()`：改变速度实现跳跃
...
public:
    ...
    Obstacle(int startX, int gapY, int gapSize);
    void update();  // 左移障碍物
    bool isColliding(int birdX, int birdY) const;  // 碰撞检测
};
```

- `update()`：每帧向左移动
- `isColliding()`：判断小鸟是否碰到障碍物
...
private:
    ...
    void generateObstacle();         // 随机生成障碍物
    bool checkCollision() const;     // 碰撞检测

public:
    Game();
    void run();                       // 游戏主循环
    void update();                    // 更新游戏状态
    void draw() const;                // 绘制游戏界面
};
```

- `run()`：主循环，调用 `update()` 和 `draw()`，处理用户输入
- `update()`：更新小鸟位置、障碍物移动和碰撞检查
- `draw()`：刷新控制台显示
...
1. **物理模拟**
   ...
   - 可以设置常量 `GRAVITY` 控制重力加速度，`JUMP_VELOCITY` 控制跳跃高度。

2. **障碍物生成**

   - 随机缺口纵坐标，保证可通过性
   - 可设置不同难度（如增加障碍物数量、减小缺口大小）

3. **控制台绘制优化**

   - 可使用 ANSI 转义序列移动光标，实现无闪烁刷新
   - 或者使用第三方库如 `ncurses`（Linux）/`conio.h`（Windows）
...
5. **拓展挑战**
   ...
   - 添加不同速度的障碍物
   - 添加小鸟动画效果（如翅膀扇动，可考虑使用字符的不断变换实现）
   - 实现多关卡模式或速度递增模式
   - 使用 `EasyX`([EasyX 中文官网](https://easyx.cn/)) 或 `SFML` 等图形库实现更丰富的图形界面

这个版本中，所有游戏逻辑通过 **面向对象的类封装**，提高了 **模块化、可维护性和扩展性**。主函数只需实例化 `Game` 类并调用 `run()`，便可启动完整游戏流程。

> 如果大家在《C语言三周入门》部分已经实现过 C语言的控制台版本 Flappy Bird，可以尝试将其重构为**面向对象版本**，提升代码质量和结构。
